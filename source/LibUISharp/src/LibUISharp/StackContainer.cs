...
namespace LibUISharp
{
    ...
    public class StackContainer : MultiContainer<StackContainer, StackContainer.ControlList, Control>
    {
        ...
        public StackContainer(Orientation orientation)
        {
            switch (orientation)
            {
                case Orientation.Horizontal:
                    Handle = new SafeControlHandle(NativeCalls.NewHorizontalBox());
                    break;
                case Orientation.Vertical:
                    Handle = new SafeControlHandle(NativeCalls.NewVerticalBox());
                    break;
                default:
        ...
        public bool IsPadded
        {
            get
            {
                isPadded = NativeCalls.BoxPadded(Handle);
                return isPadded;
            }
            set
            {
                if (isPadded != value)
                {
                    NativeCalls.BoxSetPadded(Handle, value);
                    isPadded = value;
                }
            }
        }
        ...
        public class ControlList : ControlListBase
        {
            ...
            {
                base.Add(child);
                NativeCalls.BoxAppend(Owner.Handle, child.Handle, stretches);
            }
            ...
            public new bool Remove(Control child)
            {
                ...
                {
                    NativeCalls.BoxDelete(Owner.Handle, child.Index);
                    return true;
                }
                return false;
            }
